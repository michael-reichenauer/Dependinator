...
interface IDependenciesService2
{
    bool IsShowExplorer { get; }
    bool IsShowTrees { get; }
    string Icon { get; }

    Tree2 TreeData(TreeSide side);
...
[Scoped]
class DependenciesService2(
    ISelectionService selectionService,
    IApplicationEvents applicationEvents,
    IModelService modelService,
    IPanZoomService panZoomService) : IDependenciesService2
{
    ...
    public Tree2 TreeData(TreeSide side)
    {
    ...
    public TreeItem2 ItemSelected(TreeItem2 selectedItem)
    {
        using var model = modelService.UseModel();
    ...
    Tree2 GetActiveTree(TreeSide selectedSide) =>
         selectedSide == TreeSide.Left ? leftTree : rightTree;

    static TreeItem2 SetSelectedSideItems(TreeItem2 selectedItem, Node selectedNode, Node root)
    {
    ...
    static void SetOtherSideItems(TreeItem2 selectedItem, Node selectedNode, Node root)
    {
    ...
    static void SetOtherAncestorSideItems(TreeItem2 selectedItem, Node selectedNode, Node otherNode, HashSet<NodeId> addedNodes)
    {
        var otherTree = selectedItem.Tree.OtherTree;
    ...
    static IEnumerable<Node> LineNodes(Tree2 tree, Node node)
    {
    ...
    static IEnumerable<Node> LinkNodes(Tree2 tree, Node node)
    {
        if (tree.Side == TreeSide.Left)
