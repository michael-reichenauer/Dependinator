...
- **.NET 11 GA follow-up (November 2026):** set `global.json` to the GA SDK without `allowPrerelease`, `LangVersion` to `15`, and drop `--quality preview` from the install lines in `.devcontainer/Dockerfile` and `scripts/installdevtools`. If the target framework moves to net11.0, delete `src/Dependinator.Core/Utils/UnionPolyfill.cs`, the explicit `LangVersion`, and the `dotnet-version: 10.0.x` steps in the workflows; a base-class-library `Result` type would show up as CS0104.
