            ...
            <Box style={{ width: 320, height: 330, padding: 20 }}>
                ...
                <Formik
                    ...
                    onSubmit={async (values, { setErrors, setFieldValue }) => {
                        if (createAccount) {
                            try {
                                await authenticate.createUser({ username: values.username, password: values.password })
                        ...
                        try {
                            await authenticate.connectUser({ username: values.username, password: values.password })
                            setDefaultUserName(values.username)
                        } catch (error) {
                            setFieldValue('password', '', false)
                ...
                >
                    ...
                    {({ submitForm, isSubmitting }) => (
                        <Form onKeyUp={handleEnter}>
                            ...
                            <br />
                            <FormControlLabel style={{ position: 'absolute', top: 260 }}
