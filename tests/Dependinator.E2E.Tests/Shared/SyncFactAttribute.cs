...
// Azurite running, so they only execute when E2E_SYNC=1 (set by './scripts/e2e -s').
...
public sealed class SyncFactAttribute : FactAttribute
{
    public SyncFactAttribute()
    {
        ...
        {
            Skip = "E2E tests are skipped unless E2E=1. Run them via ./scripts/e2e";
        }
        else if (Environment.GetEnvironmentVariable("E2E_SYNC") != "1")
        {
            Skip = "Sync tests need the cloud-sync stack. Run them via ./scripts/e2e -s";
        }
    }
}
