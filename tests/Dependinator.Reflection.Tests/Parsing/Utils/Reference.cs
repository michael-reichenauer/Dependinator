...
namespace Dependinator.Reflection.Tests.Parsing.Utils;
...
class Reference
{
    ...
    public static string NodeName<T>(string? memberName = null)
    {
        ...
        {
            return Dependinator.Reflection.Parsing.Assemblies.Name.GetTypeFullName(typeDefinition);
        }
        if (TryGetMember(typeDefinition, memberName, out var member))
        {
            return Dependinator.Reflection.Parsing.Assemblies.Name.GetMemberFullName(member);
        }
